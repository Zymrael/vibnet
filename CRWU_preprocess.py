# -*- coding: utf-8 -*-
"""
Created on Tue Nov 20 22:31:46 2018
...
def CWRUdata(PATH='L:/vibnet-hankooktire/CWRUdata/'):
    '''
    Returns a train DataLoader (2080 time series) and validation DataLoader (892)
    of CWRU mechanical bearing fault detection dataset. 
    The classes are balanced (baseline, ball fault, outer race, inner race)
    '''
        
    ## reading from folders
    PATH = f'{PATH}baseline/'
    baselined = utils.readFromFolder(PATH)
    PATH = f'{PATH}ball/'
    balld = utils.readFromFolder(PATH)
    PATH = f'{PATH}inner_race/'
    iraced = utils.readFromFolder(PATH)
    PATH = f'{PATH}outer_race/'
    oraced = utils.readFromFolder(PATH)
